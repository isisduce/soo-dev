...
interface WebExplorerDialogProps {
    open: boolean;
    onClose: () => void;
...
interface ExplorerDialogContentProps {
    apiServer: string;
    rootPath: string;
    initialPath: string;
}

const ExplorerDialogContent: React.FC<ExplorerDialogContentProps> = ({ apiServer, rootPath, initialPath }) => {
    const {
        currentPath,
        directories,
        currentDirectories,
    ...
    return (
        <div className={`web-filesystem-dialog ${isDragOver ? 'drag-over' : ''}`}>
            {/* 드래그 앤 드롭 오버레이 */}
            {isDragOver && (
...
export const WebExplorerDialog: React.FC<WebExplorerDialogProps> = ({
    open,
    onClose,
    onFocus,
    title,
